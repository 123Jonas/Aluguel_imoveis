...
const Messages = () => {
  ...
  const fetchConversations = async () => {
    try {
      ...
      const response = await axios.get(`${apiUrl}/api/messages/tenant/conversations`, {
        headers: {
      ...
      if (response.data.success) {
        ...
        const validConversations = response.data.data.conversations.map(conv => {
          ...
          return {
            ...
            landlordName: conv.landlordName || 'Proprietário sem nome',
    ...
    setConversations(prev => {
      ...
      if (!updatedConversations.find(conv => conv.chatId === message.chatId)) {
        updatedConversations.unshift({
          ...
          landlordName: message.senderName,
          lastMessage: message.message,
          timestamp: message.timestamp,
          unread: true,
          messages: [message]
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      if (!selectedConversation.chatId || !selectedConversation.landlordId || !selectedConversation.propertyId) {
      ...
      console.log('Enviando mensagem:', {
        ...
        receiverId: selectedConversation.landlordId,
  ...
  const handleSelectConversation = async (conversation) => {
    try {
      ...
      if (!conversation.chatId || !conversation.landlordId || !conversation.propertyId) {
      ...
      const response = await axios.get(`${apiUrl}/api/messages/tenant/messages/${conversation.chatId}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
                    ...
                    >
                      <div className="d-flex align-items-center">
                        ...
                        <div className="flex-grow-1">
                          ...
                          <p className="text-muted small mb-0">{conversation.landlordName}</p>
                ...
                <Card.Header className="bg-white border-bottom">
                  <div className="d-flex align-items-center">
                    ...
                    <div>
                      ...
                      <p className="text-muted small mb-0">{selectedConversation.landlordName}</p>
                    </div>
                  </div>
                </Card.Header>
                <Card.Body className="p-0">
                    ...
                    {selectedConversation.messages.map((message, index) => (
                      <div
                        ...
                        className={`message-wrapper mb-3 ${
                          message.senderId._id === localStorage.getItem('userId') ? 'text-end' : 'text-start'
                        }`}
                      >
                        <div
